...
local function tryRechargeKi()
    -- No DBRage, recarregar Ki é segurar a tecla Q
    -- Simula pressão da tecla via VirtualInputManager (disponível em executors)
...
-- Encontra o Hitbox do NPC mais próximo (tag "Hitbox")
local function getNearestHitbox()
    ...
    -- Procura em todo o workspace partes com tag "Hitbox"
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("BasePart") and obj:HasTag("Hitbox") then
            -- Verifica se o owner (NPC) ainda está vivo
            local ownerVal = obj:FindFirstChild("Owner")
            local ownerModel = ownerVal and ownerVal.Value
            if ownerModel then
...
local FARM_INTERVAL = 0.08 -- clica ~12x por segundo
...
local function startAutoFarm()
    ...
    autoFarmConn = RunService.Heartbeat:Connect(function(dt)
        ...
        -- Verifica se stat atual atingiu limite → passa pro próximo
        ...
        -- A cada FARM_INTERVAL segundos: acha o hitbox mais próximo, teleporta e clica
        if autoFarmTick >= FARM_INTERVAL then
            autoFarmTick = 0

            local hitbox, dist = getNearestHitbox()
            if hitbox then
                local hrp = getHRP()
                if hrp then
                    -- Teleporta junto ao hitbox se estiver longe
                    if dist > 8 then
                        hrp.CFrame = CFrame.new(hitbox.Position) * CFrame.new(0, 0, 3.5)
                    end

                    -- Simula clique do mouse no hitbox (é assim que o jogo registra hit)
                    local fakeHit = hitbox.Position
                    mouse.Hit = CFrame.new(fakeHit)
                    mouse.Target = hitbox

                    -- mouse1click dispara o evento de ataque no jogo
                    mouse:button1Down()
                    task.defer(function()
                        mouse:button1Up()
                    end)
                ...
                -- Nenhum NPC encontrado — loga de vez em quando
                if math.floor(state.farmElapsed) % 5 == 0 and autoFarmTick < 0.01 then
                    addLog("Nenhum NPC encontrado perto. Aguardando...")
                end
            end
        end
    end)
...
local function startAutoZenkai()
    ...
    -- Zenkai no DBRage acontece automaticamente quando o personagem
    -- morre e respawna com HP baixo — não há tecla ou remote cliente-side
    -- Esta função mantém o toggle ativo para referência futura
    addLog("Auto Zenkai ativado (requer morte natural no jogo)")
end
